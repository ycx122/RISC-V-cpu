本项目 fork 自 https://github.com/riscv/riscv-compliance，并做了适配以便运行在当前处理器上。

使用方法：

1. 编译：`make`
2. 重新编译：先执行 `make clean`，再执行 `make`

默认工具链前缀已经改为指向同级目录下的 `../toolchain/riscv64-unknown-elf-gcc-8.3.0-2020.04.0-x86_64-linux-ubuntu14/bin/riscv64-unknown-elf-`。

编译生成的文件会输出到 `build_generated` 目录，该目录属于可重复生成内容，默认不保留在仓库中。

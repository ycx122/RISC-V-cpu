...
- `axil_interconnect`：1 主→6 从的 AXI4-Lite 地址译码器（ROM / RAM / LED / KEY / CLINT / UART），替代了原来的 `addr2c.v`
...
- `rodata`：ROM 数据读控制（只返回 32 位对齐字，字节抽取 / 符号扩展交给 master bridge）
- `cpu_uart`：控制串口
- `clnt`：定时器/中断相关寄存器
...
总线采用 **AXI4-Lite 标准**，因此可以把 Xilinx MIG 的 AXI-Lite facade、`axi_uart16550`、`axi_qspi`、`axi_dma` 的 config 口等任何 AXI4-Lite IP 直接挂在互连上（只需要在 `axil_interconnect.v` 增加一条地址译码条目和一组从口 bundle）。

同时，指令取值由 `cpu_jh` 通过 `ram_c` 直接访问指令存储体（不走总线），在现在这一版仍保持 1 周期 BRAM 读。
...
地址译码逻辑位于 `rtl/bus/axil_interconnect.v`（每个从口都挂在 1-master→6-slave 的 AXI4-Lite 互连上）。按照代码中的范围，SoC 地址空间如下：
